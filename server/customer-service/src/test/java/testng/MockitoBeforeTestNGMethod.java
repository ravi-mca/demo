...
public class MockitoBeforeTestNGMethod {
  ...
  private WeakHashMap<Object, Boolean> initializedInstances = new WeakHashMap<>();
  ...
  private void initializeCaptors(Object instance) {
    List<InstanceField> instanceFields =
        Fields.allDeclaredFieldsOf(instance).filter(annotatedBy(Captor.class)).instanceFields();
    instanceFields.forEach(this::setInstanceField);
  }

  private void setInstanceField(InstanceField instanceField) {
    instanceField.set(new CaptorAnnotationProcessor().process(instanceField.annotation(Captor.class),
