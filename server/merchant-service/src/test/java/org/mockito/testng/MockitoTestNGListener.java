 ...
 * {@link org.mockito.MockitoAnnotations#initMocks(Object)} is not necessary</strong>. <strong>Note
 * :</strong> With TestNG, mocks are initialized before any TestNG method, either a
 * <em>configuration
 *         method</em> (&#064;BeforeMethod, &#064;BeforeClass, etc) or a <em>test</em> method, i.e.
 * mocks are initialized once only once for each test instance.</li>
 * <li>As mocks are initialized only once, they will be reset after each <em>test method</em>. See
 * javadoc {@link org.mockito.Mockito#reset(Object[])}</li>
 * <li>Validates framework usage after each test method. See javadoc for
 * {@link org.mockito.Mockito#validateMockitoUsage()}.</li>
 * </ul>
 *
 * <p>
 * The listener is completely optional - there are other ways you can get &#064;Mock working, for
 * example by writing a base class. Explicitly validating framework usage is also optional because
 * it is triggered automatically by Mockito every time you use the framework. See javadoc for
...
// This code is available at mockito git [https://github.com/mockito/mockito].
// Though there is no existing maven artifact to get it.
